...
def _resolve_text_encoder_path(text_encoder):
    if os.path.isdir(text_encoder):
        return text_encoder
    cache_home = os.environ.get('HF_HOME')
    candidates = []
    if cache_home:
        candidates.extend([
            os.path.join(cache_home, "hub", "models--bert-base-uncased", "snapshots"),
            os.path.join(cache_home, "transformers", "bert-base-uncased"),
        ])
    candidates.extend([
        os.path.expanduser("~/.cache/huggingface/hub/models--bert-base-uncased/snapshots"),
        os.path.expanduser("~/.cache/huggingface/transformers/bert-base-uncased"),
    ])
        ...
        if os.path.isdir(root):
            ...
            for path in entries:
                if os.path.isfile(os.path.join(path, "vocab.txt")):
                    return path
            if os.path.isfile(os.path.join(root, "vocab.txt")):
                return root
    return text_encoder
...
def _build_tokenizer_with_fallback(text_encoder, local_files_only=False):
    ...
    try:
        ...
        try:
            return BertTokenizerFast.from_pretrained(resolved)
        except Exception:
...
def main_worker(gpu, args, config):
    ...
    if args.distributed:
        # Some branches are intentionally unsupervised (bbox/fake-text), so a subset
        # of parameters can be unused in backward on each step.
